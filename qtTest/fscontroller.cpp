...
void FSController::fetchFrame()
{
	...
	cv::imshow("extractedlaserLine2",result);
	cv::waitKey(1);
...
void FSController::scanThread()
{
    ...
    for(double i=0; i<360 && scanning==true; i+=stepDegrees){
   ...
   cv::waitKey( 100);//  
	...
	//	cv::moveWindow("extractedlaserLine", 50,80);
        ...
        //here magic happens
        vision->putPointsFromFrameToCloud(laserOff, laserOn, yDpi, 0,JD);
        //update gui
...
void FSController::scanThread2()
{
	...
	for(double j=0; j<360 && scanning==true; j+=turntableStepSize){
		...
		for(double i=laserSwipeMin; i<laserSwipeMax && scanning==true; i+=laserStepSize){
			...
			vision->putPointsFromFrameToCloud(laserOff, laserOn, 5, 0,0);
			geometries->setPointCloudTo(model->pointCloud);
			mainwindow->redraw();
...
bool FSController::detectLaserLine()
{
	...
	/*FSPoint px = vision->convertCvPointToFSPoint(laser->mpx);

	laser->setLaserPointPosition(px,laser->mpx);
...
HHHH:
	...
	FSPoint p = vision->detectLaserLine( laserOffFrame, laserOnFrame, threshold ,&mp);

	if( mp.x>640 || mp.y>480 || p.x == 0.0  ){

		goto HHHH;
		return false;}


	laser->setLaserPointPosition(p,mp);
