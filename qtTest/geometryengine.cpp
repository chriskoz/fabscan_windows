...
void GeometryEngine::setPointCloudTo(pcl::PointCloud<pcl::PointXYZRGB>::Ptr pointcloud)
{
    ...
    VertexData *vertices;//[19999];//];
	vertices =new VertexData[xx];
    //VertexData vertices[9999];
    GLuint *indices;//[19999];//pointcloud->points.size()];
...
void GeometryEngine::setSurfaceMeshTo(pcl::PolygonMesh surfacemesh, pcl::PointCloud<pcl::PointXYZRGB>::Ptr pointCloud)
{
    ...
    VertexData vertices[9999];//cloud.points.size()];
    //VertexData vertices[9999]; //TODO: solve this in a nicer way
    GLuint indices[9999];//surfacemesh.polygons.size()*3];
