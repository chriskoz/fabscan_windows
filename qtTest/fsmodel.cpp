...
//    asprintf(&command,"cd %s; ./powercrust -i %s -R 1.5 -B -m 10000", resPath, "pc.pts");
	char command[1024];
	sprintf(command,"cd %s; ./powercrust -i %s -R 1.5 -B -m 10000", resPath, "pc.pts");
    ...
    sprintf(command,"cd %s; ./orient -i pc.off -o final.off",resPath);
    sysRet = system(command);
    cerr << command << " system: " << sysRet << endl;

    char offFilePath[1024];
    sprintf(offFilePath,"%sfinal.off",resPath);
