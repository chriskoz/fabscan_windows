...
{ 
  // initialize the serial port
   ...
   pinMode(LASER_PIN, OUTPUT);
    pinMode(LASER_PIN+1, OUTPUT);
     pinMode(LASER_PIN+2, OUTPUT);
      ...
      case ACTION_BYTE:
          ...
          switch(incomingByte){    //this switch always handles the first byte
            ...
            case TURN_LASER_OFF:
              ...
              digitalWrite(LASER_PIN+1, LOW);    // turn the LASER off
              digitalWrite(LASER_PIN+2, LOW);    // turn the LASER off
              break;
            case TURN_LASER_ON:
              digitalWrite(LASER_PIN, HIGH);   // turn the LASER on
              digitalWrite(LASER_PIN+1, HIGH);   // turn the LASER on
              digitalWrite(LASER_PIN+2, HIGH);   // turn the LASER on
              break;
